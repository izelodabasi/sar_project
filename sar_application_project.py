...
class ZodiacSwitch(app_manager.RyuApp):
	...
	def __init__(self, *args, **kwargs):
		...
		self.rules = get_rules(self, 100)

		self.CP = table_cross_producting(self, self.rules)

		self.BM = table_bit_map(self, self.rules)
	...
	# HANDLE ARP PACKETS--------------------------------------------
		...
		if ip4_pkt:
			...
			self.logger.info("Packet in switch: %s, source IP: %s, destination IP: %s, From the port: %s, protocol: %s", dpid_src, src_ip, dst_ip, in_port, proto)
			# self.logger.info("Packet in switch: %s, source MAC: %s, destination MAC: %s, From the port: %s", dpid_src, src, dst, in_port)
				

			a = datetime.datetime.now()
			action_rule = cross_producting_classification(self, self.CP, src_ip, dst_ip, proto, sport, dport)
			#action_rule = bit_map_classification(self, self.BM, src_ip, dst_ip, proto, sport, dport)
